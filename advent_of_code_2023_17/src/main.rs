...
impl Axis {
    fn opposite(&self) -> Axis {
        match self {
            Axis::X => Axis::Y,
            Axis::Y => Axis::X,
            _ => panic!("XY axis does not have an opposite.")
        }
    }
...
fn dijkstra(graph: &Graph<Coord, usize, Directed>, start: NodeIndex, goal: NodeIndex, axis: Axis) -> usize {
    ...
    while let Some(Reverse(node)) = priority_queue.pop() {
        ...
        for neighbor in graph.neighbors(node_index) {
            ...
            let edge_weight = graph.edges_connecting(node_index, neighbor).next().unwrap().weight();
            ...
            let next_axis = if x == coord.0 { Axis::Y } else { Axis::X };
            update_weight(&mut priority_queue, neighbor, potential_weight, next_axis);
        }
    }
    usize::max_value()
}
...
fn min_heat_loss(file: &str) -> usize {
    ...
    dijkstra(&graph, start, goal, Axis::XY)
}

fn main() {
    println!("Answer for example: {}", min_heat_loss("example.txt"));
    println!("Answer for puzzle input: {}", min_heat_loss("input.txt"));
}